...
export class NotificationService {
    public static prepareNotifications(trackedItems: TrackedItemWithUser[], shopSkins: SkinsShop) {
        return trackedItems.map(item => {
            const skinData = shopSkins.outfits.find((entry: Entry) => entry.brItems && entry.brItems[0]?.id === item.skinId) as Entry;
