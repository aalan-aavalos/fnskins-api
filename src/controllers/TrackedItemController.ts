...
class TrackedItemController {
    ...
    public create = async (req: Request, res: Response): Promise<void> => {
        try {
            const trackedItem = await this.trackedItemService.createTrackedItem(req.body);
    ...
    public findOne = async (req: Request, res: Response): Promise<void> => {
        try {
            const trackedItem = await this.trackedItemService.getTrackedItemById(req.params.id);
            if (!trackedItem) {
    ...
    public findOneByUserId = async (req: Request, res: Response): Promise<void> => {
        try {
            const trackedItemsByUserId = await this.trackedItemService.getTrackedItemByUserId(req.params.userId);
    ...
    public update = async (req: Request, res: Response): Promise<void> => {
        try {
            const updatedTrackedItem = await this.trackedItemService.updateTrackedItem(req.params.id, req.body);
    ...
    public delete = async (req: Request, res: Response): Promise<void> => {
        try {
            const deleted = await this.trackedItemService.deleteTrackedItem(req.params.id);
            if (!deleted) {
                res.status(404).json({ error: 'TrackedItem not found' });
                return;
            }
